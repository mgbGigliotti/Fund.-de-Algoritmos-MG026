...
def load_usuario():
    ...
    for linha in arquivo:
        ...
        if len(dados) == 3: # checa valor correto na linha
            id_usuario = int(dados[0]) # transforma um valor do txt em numero inteiro
            nome = dados[1]
            senha = dados [2]
...
def load_filmes():
    ...
    for linha in arquivo:
        ...
        if len(dados) == 5: # checa valor correto na linha
            id_filme = int(dados[0]) # transforma um arquivo do txt em um numero inteiro
            nome_filme = dados[1]
            duracao = int(dados[2])
            ano = int(dados[3])
            sinopse = dados[4]
...
def filme(id_filme): 
    for filme in filmes: # percorre os filmes no txt e confirma a existência
        if filme[0] == id_filme:
            return True

    return False # nao tem no catalogo

def catalogo(): # mostra todos os filmes 
    print("\n   CATÁLOGO DE FILMES")
    for filme in filmes:
        print(f"{filme[0]} - {filme[1]}") #id e nome
...
def buscar_filmes(usuario_logado): 
    ...
    if opcao == "1":                                                            
        nome_busca = input("Digite o nome do filme: ").strip().lower() #strip remove espaços em branco
        encontrado = False
        for filme in filmes:
    ...
    for linha in arquivo:
        ...
        if len(dados) == 2:
            ...
            if usuario_salvo == id_usuario and filme_salvo == id_filme: # não salva o mesmo filme duas vezes
                ...
                return
    arquivo.close()
    arquivo = open("historico.txt","a") # 'a' é o append, para adicionar abaixo sem apagar outros
...
def descurtir_filme(id_usuario,id_filme):
    if not filme_curtido(id_usuario, id_filme):
        ...
        return
    arquivo = open("curtidos.txt", "r")
    linhas = arquivo.readlines() # torna cada linha de arquivo txt em uma lista de string
    arquivo.close()

    arquivo = open("curtidos.txt","w") # reescreve o que deve continuar
    for linha in linhas:
        dados = linha.strip().split(";")
        if len(dados) == 2:
            usuario_salvo = int(dados[0])
...
def criar_lista(usuario_logado):
    id_usuario = usuario_logado[0] # id do usuario logado
    nome_lista = input("Digite o nome da nova lista: ").strip() 
    arquivo = open("listas_favoritos.txt", "r")
    linhas = arquivo.readlines() # le todas as linhas do arquivo e transforma em lista de string

    arquivo.close()
    id_lista = len(linhas) + 1 # cria automaticamente um ID para a lista
...
def mostrar_listas(usuario_logado):
    ...
    print("\n   SUAS LISTAS")
    arquivo = open("listas_favoritos.txt", "r")
    encontrou = False # controla o encontro das listas
    for linha in arquivo:
        dados = linha.strip().split(";") 
        if len(dados) == 3:
            id_lista = int(dados[0]) # ID da lista
            usuario_salvo = int(dados[1]) # dono da lista
            nome_lista = dados[2] # nome da lista

            if usuario_salvo == id_usuario: # mostra apenas listas do usuário
    ...
    for linha in linhas:
        ...
        if len(dados) == 3:
            ...
            if lista_id == id_lista and usuario_salvo == id_usuario:
                arquivo.write(f"{lista_id};{usuario_salvo};{novo_nome}\n") # para reescrever o nome da lista 
            else:
                arquivo.write(linha) # mantém listas normais
...
def excluir_lista(usuario_logado):
    ...
    arquivo.close()
    arquivo = open("listas_favoritos.txt", "w")
    encontrou = False # controla se encontrou a lista

    for linha in linhas:
        dados = linha.strip().split(";")
        if len(dados) == 3:
            lista_id = int(dados[0]) #lista_id esta vindo do arquivo
            usuario_salvo = int(dados[1])
    ...
    for linha in linhas:
        ...
        if len(dados) == 2:
            lista_id = int(dados[0])
            if lista_id == id_lista:
                continue # remove filmes da lista excluída
        arquivo.write(linha)
    arquivo.close()
...
def mostrar_filmes_da_lista(id_lista):
    ...
    for linha in arquivo:
        ...
        if len(dados) == 2:
            lista_id = int(dados[0])
            id_filme = int(dados[1])
            if lista_id == id_lista: # novamente, lista_id vem do arquivo e id_lista o usuario digita
                for filme in filmes: 
                    if filme[0] == id_filme:
                        print(f"{filme[0]} - {filme[1]}")
                        encontrou = True
...
def lista_pertence_usuario(id_usuario, id_lista): #  função para verificar que o usuario edite apenas suas listas
    arquivo = open("listas_favoritos.txt", "r")
    for linha in arquivo:
        dados = linha.strip().split(";")
        if len(dados) == 3:
...
def adicionar_filme_lista(usuario_logado):
    id_usuario = usuario_logado[0]
    mostrar_listas(usuario_logado)  # mostra listas
    catalogo()  
    id_lista = input("\nDigite o ID da lista: ").strip()
    id_filme = input("Digite o ID do filme: ").strip()
    ...
    id_lista = int(id_lista)
    id_filme = int(id_filme)
    if not lista_pertence_usuario(id_usuario, id_lista):
        print("Essa lista não existe ou não pertence a você.")
        return
    if not filme(id_filme):     # verifica se filme existe
...
def remover_filme_lista(usuario_logado):
    ...
    if not lista_pertence_usuario(id_usuario, id_lista):
        print("Essa lista não existe ou não pertence a você.")
        return
    mostrar_filmes_da_lista(id_lista)   # mostra filmes da lista
    ...
    arquivo.close()
    arquivo = open("itens_listas.txt", "w")
    removeu = False     # controla se removeu
